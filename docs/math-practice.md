...
For “Tìm x”, a first solution row that exactly repeats the question equation is omitted when further steps exist. Trailing domain annotations stay in the question; transformed equations, explanations and lone answers are preserved. Worksheet line counts use the same continuation logic.

Numeric/fraction “Tìm x” PDF solutions show authored equation steps directly, without a separate “Đáp án” line or “Cách làm” label. Necessary reasoning and domain notes remain. This shared rule applies to compact and full-width layouts; multiple-choice, written responses and explicit solution-style overrides retain their existing presentation.

Calculation question labels and PDF continuation matching share `calculationExpression` in `format.ts`. It supports ordinary “Tính”, reasonable/quick calculation and expression-value instructions, numbered practice prefixes, and word-only instructions ending in a colon. Add future non-delimited instruction variants there, not in separate renderers. Only a matching initial numeric expression is removed; different first transformations and conditions remain intact. Regression tests cover shared labels, solution output, and reasonable-calculation examples.
...
Teachers can add, reorder, edit or remove extra questions. Optional fields: `group` (foundation/skills/application/challenge), `skill`, `difficulty` (easy/medium/hard), and legacy `workspace` (small/medium/large, retained for compatibility). Worksheet writing space is now automatic: dotted rows are sized from the formatted model solution, including wrapped text and stacked fractions, with no spare row and a one-row minimum. Pure calculations omit an initial expression matching the question before measuring; columns share the largest writing area in their row. Number-line tasks reserve at least five rows. Choice and table tasks have no extra writing rows. The old fixed workspace size does not override this sizing. `written` questions are supported only in extra practice and require a model `solution` and `criteria` array; they are explicitly self-assessed, never automatically marked correct. Excel `criteria` cells contain JSON arrays. Existing packs without extra questions remain valid.
...
The equality/transposition lesson now contains the 33 extra questions moved from the exponent lesson in six lettered task groups: basic
...
PDF numeric calculation-only solutions omit the redundant answer and method
labels. They display the expression and equality steps directly, with aligned
...
Nine built-in lessons now open an interactive activity in **Khám phá**. Grade 3 covers missing addition/subtraction components, multiplication/division groups, unit fractions, measurement and midpoints. Grade 7 covers rational numbers, number lines, powers, and equality/transposition. Students can change quantities, distribute objects, fill a jug, combine weights, move a point and compare positions on a number line. Activities have three situations, except equality/transposition which has eleven to cover the full operations summary; each includes explanatory feedback, keyboard/touch controls and a short reasoning prompt. Answer choices check immediately, and measurement controls update the model and feedback together. Existing explanations remain available through **Xem giải thích và ví dụ tham khảo**; teaching mode treats the activity as the first exploration item before the original blocks.

Teachers can choose or disable an activity through **Hoạt động khám phá tương tác** in lesson metadata. The optional `interactiveLab` field accepts `none`, `sharing`, `number-line`, `powers`, `missing-parts`, `equal-groups`, `measurement`, `midpoint`, `rational` or `equality`, and survives JSON import/export, duplication and sharing. Older built-in lessons resolve a default by ID and grade at render time, so simply opening them does not rewrite saved lessons or invalidate progress. Duplicates store their resolved choice explicitly. These activities are online exploration tools; PDF practice continues to use the authored exercises.
...
Typed exercise answers automatically check after an 800 ms pause once all required fields are filled. The shared Exercise component applies this to existing, imported and newly authored lessons, in lesson and extra-practice views. Enter checks immediately; choices still check immediately. Editing clears stale feedback and restarts the timer. Incomplete signs, decimal separators, fractions and missing units wait; composition, help, hidden teaching questions and unmount cancel pending checks. Written responses retain manual self-review. The existing `assisted` field continues to mean “not correct on the first attempt without help,” including an incorrect submitted answer; it is not a count of hints used.

The Grade 7 semester 1 lesson **Đẳng thức và quy tắc chuyển vế** includes an equality warm-up, a balance activity, two sign-and-answer rounds (integers and fractions), nine teaching blocks and 49 exercises (including 33 find-x questions held only in this lesson). Existing libraries receive it once through `lima-math-equality-v1`; edited copies are preserved. Research and teaching rationale: [math-equality-lesson.md](math-equality-lesson.md).
